...
export default function DInvictaMasterPro() {
  ...
  useEffect(() => {
    ...
    if (leftX === 1 && leftUnits === 0 && rightUnits === 5) {
      setMsg("🎉 MISSION COMPLETE! Result saved to dashboard.");
      handleFinalSave();
    }
  ...
  async function handleFinalSave() {
    ...
    const gap = timeTaken > 60 ? "Slow Pace: Check Integer Fluency" : "None";
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in zoom-in duration-500">
            <div className="bg-white p-12 rounded-[3.5rem] border-2 border-slate-100 shadow-2xl">
              <header className="text-center mb-12">
                ...
                <div className="mt-2 inline-block bg-blue-50 px-4 py-1 rounded-full text-blue-600 font-mono font-bold text-lg underline decoration-blue-200">3x + 15 = 30</div>
              </header>

              <div className="h-64 flex flex-col items-center justify-center mb-16 relative">
                <div className="w-full h-2 bg-slate-800 rounded-full transition-transform duration-700 shadow-sm" style={{ transform: `rotate(${balance}deg)` }}>
          ...
          <div className="max-w-6xl mx-auto space-y-10">
            <div className="flex justify-between items-end">
              ...
              <div className="flex gap-4">
                <button onClick={fetchSubmissions} className="bg-white border-2 px-6 py-2 rounded-xl font-bold hover:bg-slate-50 transition">Refresh</button>
                <button onClick={downloadExcel} className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-slate-200">Generate Excel</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-8">
               ...
               <div className="bg-emerald-600 p-8 rounded-[2rem] text-white shadow-xl"><p className="text-xs font-bold text-emerald-200 uppercase tracking-widest">Active Status</p><p className="text-xl font-black mt-2 underline decoration-emerald-400">Live Feedback On</p></div>
            </div>

            <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden">
               <table className="w-full text-left">
                 <thead className="bg-slate-50 border-b text-[10px] font-black uppercase text-slate-400 tracking-widest">
                   <tr><th className="p-6">Student</th><th className="p-6">Score</th><th className="p-6">Diagnosis</th><th className="p-6">Time (Sec)</th><th className="p-6">Submission Date</th></tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                   {submissions.map((s, i) => (
                     <tr key={i} className="hover:bg-slate-50 transition-colors">
                       ...
                       <td className={`p-6 text-xs font-bold ${s.misconception_detected !== 'None' ? 'text-red-500 underline' : 'text-slate-400'}`}>{s.misconception_detected}</td>
