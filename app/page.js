...
import { 
  ...
  Target, BookOpen, ChevronRight, Hash, AlertCircle, Loader2
} from 'lucide-react';

export default function DInvictaPowerDash() {
  ...
  // --- DATA PIPELINE: THE FIX ---
  async function fetchSubmissions() {
    setIsLoading(true);
    console.log("D-INVICTA: Fetching latest intelligence...");
    try {
  ...
  // Effect to load data immediately when entering dashboard
  useEffect(() => {
    if (viewMode === 'teacher_dash') {
      fetchSubmissions();
    }
  }, [viewMode]);
  ...
  useEffect(() => {
    ...
    if (activeGrade === 'Grade 2' && g2.hundreds === 2 && g2.tens === 4 && g2.ones === 5) handleMastery("Mastered: Visual Logic");
    const tilt = g7.target - ((g7.x * 5) + g7.units);
    setG7(prev => ({ ...prev, balance: tilt }));
    if (activeGrade === 'Grade 7' && g7.x === 1 && g7.units === 0 && g7.target === 5) handleMastery("Mastered: Algebraic Flow");
  }, [g2, g7]);
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(wb, "D-INVICTA_Intelligence_Report.xlsx");
  };
    ...
    return (
      <div className="h-screen w-screen bg-[#0f172a] flex items-center justify-center p-6">
        <div className="bg-white max-w-md w-full rounded-[3.5rem] p-12 shadow-2xl text-center border-t-[12px] border-blue-600 animate-in zoom-in">
          ...
          <input type="text" placeholder="Full Name" className="w-full bg-slate-50 border-2 rounded-2xl px-6 py-4 mb-6 font-black text-slate-700 outline-none focus:border-blue-500 transition-all" onChange={(e)=>setTempName(e.target.value)}/>
    ...
    <div className="flex h-screen bg-[#f1f5f9] font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-80 bg-[#0f172a] text-white flex flex-col p-8 shadow-2xl relative z-20">
        ...
        <div className="flex-1 overflow-y-auto space-y-8 pr-2">
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase mb-4 tracking-widest text-white/40">Mastery Track</p>
            <div className="grid grid-cols-2 gap-2">
              {["Grade 2", "Grade 7"].map(g => (
                <button key={g} onClick={()=>{setActiveGrade(g); setActiveTopic(curriculum[g][0]); setIsComplete(false);}} className={`px-2 py-3 rounded-xl text-[10px] font-black transition-all ${activeGrade === g ? 'bg-blue-600 shadow-lg' : 'bg-slate-800 text-slate-500 hover:bg-slate-700'}`}>{g}</button>
              ))}
            </div>
          </div>
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase mb-4 tracking-widest text-white/40">Topics</p>
            {curriculum[activeGrade].map(topic => (
              <button key={topic} onClick={()=>{setActiveTopic(topic); setIsComplete(false);}} className={`w-full text-left p-4 rounded-2xl text-xs font-black mb-2 transition-all ${activeTopic === topic ? 'bg-slate-800 text-blue-400 border-l-4 border-blue-500' : 'text-slate-500'}`}>{topic}</button>
            ))}
          </div>
        </div>
        <div className="pt-8 border-t border-slate-800 space-y-3">
          <button onClick={()=>setViewMode('student_portal')} className={`w-full flex items-center gap-3 p-4 rounded-2xl text-xs font-black uppercase transition-all ${viewMode === 'student_portal' ? 'bg-blue-600 shadow-lg' : 'text-slate-500'}`}><Activity size={18}/> Home Portal</button>
          <button onClick={()=>setViewMode('teacher_dash')} className={`w-full flex items-center gap-3 p-4 rounded-2xl text-xs font-black uppercase transition-all ${viewMode === 'teacher_dash' ? 'bg-purple-600 shadow-lg' : 'text-slate-500'}`}><LayoutDashboard size={18}/> Teacher Hub</button>
          <button onClick={()=>window.location.reload()} className="w-full text-center text-[10px] font-black text-slate-600 uppercase pt-4 hover:text-white transition">Logout</button>
        </div>
      </aside>
        ...
        {isComplete && viewMode === 'student_portal' && (
          <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6">
            <div className="bg-white rounded-[4rem] p-12 text-center shadow-2xl max-w-lg w-full border-b-[12px] border-emerald-500 animate-in zoom-in">
               ...
               <h3 className="text-4xl font-black text-slate-800 mb-2">Unconquerable!</h3>
               <p className="text-slate-500 font-bold mb-10 italic">Mathematics mastery confirmed. Data synced to hub.</p>
               <button onClick={()=>{setIsComplete(false); setStartTime(Date.now());}} className="bg-blue-600 text-white w-full py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl hover:bg-slate-900 transition flex items-center justify-center gap-3"><RefreshCw size={20}/> New Mission</button>
            </div>
          </div>
        )}
            ...
            {activeGrade === 'Grade 2' ? (
              <div className="bg-white p-12 rounded-[4rem] shadow-xl border-2 border-slate-100 text-center">
                ...
                <div className="flex justify-center items-end gap-6 h-56 mb-12 bg-slate-50 rounded-[3rem] p-10 border-2 border-dashed">
                    <div className="flex flex-wrap-reverse w-44 gap-1 content-start">{[...Array(g2.hundreds)].map((_, i) => <div key={i} className="w-12 h-12 bg-red-500 rounded-lg shadow-md border-2 border-red-700 flex items-center justify-center text-[10px] text-white font-black">100</div>)}</div>
          ...
          <div className="max-w-7xl mx-auto space-y-10 animate-in slide-in-from-bottom-8 duration-700">
             <div className="grid grid-cols-3 gap-8">
                <div className="bg-white p-10 rounded-[3rem] border shadow-sm border-b-[8px] border-blue-500">
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Class Performance</p>
                   <p className="text-6xl font-black text-slate-900">92<span className="text-2xl text-blue-500">%</span></p>
                   <div className="mt-4 flex items-center gap-2 text-emerald-600 font-bold text-xs"><TrendingUp size={16}/> System Health: Optimal</div>
                </div>
                
                <button 
                  onClick={() => setIsLive(!isLive)}
                  className={`${isLive ? 'bg-emerald-600 shadow-emerald-500/40' : 'bg-slate-400 shadow-slate-200'} p-10 rounded-[3rem] text-white shadow-2xl text-left transition-all hover:scale-[1.02] active:scale-95`}
                >
                ...
                <button 
                  ...
                  className="bg-[#0f172a] p-10 rounded-[3rem] text-white shadow-2xl text-left group hover:bg-black transition-all"
                >
                   ...
                   <p className="text-2xl font-black mt-1 uppercase underline decoration-blue-500/20">Get Excel Report</p>
                </button>
             </div>

             <div className="bg-white rounded-[3.5rem] border-2 border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
                <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                   <h4 className="font-black text-slate-800 uppercase text-xs tracking-[0.2em]">Student Intelligence Table</h4>
                   <button onClick={fetchSubmissions} className="flex items-center gap-2 text-blue-600 font-black text-[10px] uppercase hover:underline">
                      <RefreshCw size={14} className={isLoading ? 'animate-spin' : ''}/> Manual Refresh
                   </button>
                </div>
                <table className="w-full text-left">
                  <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 border-b">
                    <tr><th className="p-10">Student Identity</th><th className="p-10">Topic Path</th><th className="p-10 text-center">Score</th><th className="p-10">Intelligence Diagnosis</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {submissions.length > 0 ? submissions.map((s, i) => (
                      ...
                      <tr><td colSpan="4" className="p-32 text-center text-slate-300 font-black italic text-xl tracking-tighter uppercase">
                        {isLoading ? 'Decrypting Database...' : 'Waiting for student interaction...'}
                      </td></tr>
                    )}
                  </tbody>
                </table>
